...
with st.sidebar.expander("Scorecard 도움말", expanded=False):
    st.markdown(
        "- 거래소는 **수수료**보다 **신뢰**를 우선합니다. 신뢰 붕괴는 뱅크런으로 이어집니다.\n"
        "- **거절 핵심 3가지**: 덤핑 구조(과도한 초기 유통/집중 물량), 유동성 고갈(거래량·오더북 약함), 법적 리스크(증권성/AML).\n"
        "- 내부 심사에서는 **덤핑 테스트**와 **유동성 스트레스 테스트**를 진행합니다. 회복 불가 판정이면 즉시 거절/상폐됩니다.\n"
        "- 이 점수는 위 리스크를 **사전 경고**하는 지표입니다. 경고/위험이 뜨는 설정으로는 상장 신청을 권장하지 않습니다.\n"
        "- 목표: **Status: Stable** 및 **Legal Check: Pass**가 유지되도록 값 조정 후, 그 설정을 상장 서류에 반영."
    )

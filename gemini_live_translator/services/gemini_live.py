...
def build_risk_prompt(
    ...
    return (
        ...
        "native speaker would never produce. Korean example: ASR often turns "
        "'실시간' (real-time) into '7시간/칠시간' (ㅅ↔ㅊ confusion) — but Korean "
        "speakers read durations with NATIVE numerals ('일곱 시간'), never "
        "'칠시간', so an utterance transcribed as '칠시간/7시간' where a duration "
        "was not being discussed almost certainly means '실시간'. Apply the same "
        "reasoning to other sino-vs-native numeral readings and common "

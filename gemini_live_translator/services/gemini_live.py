...
def build_risk_prompt(
    ...
    return (
        ...
        "context, judge if the utterance was likely MIS-RECOGNIZED. Apply a "
        "PLAUSIBILITY CHECK to proper nouns especially: if a person/company/"
        "product name was transcribed as a common word that cannot plausibly be "
        "a name (e.g. a Korean self-introduction rendered as '내 이름은 안녕' — "
        "'안녕' is a greeting, never a name), that IS a mis-recognition. When "
        "the business/industry context or glossary below contains the correct "
        "term or name, use it to CORRECT the sentence; otherwise state what was "
        "probably meant and, if genuinely irrecoverable, suggest politely asking "
        "the speaker to repeat the specific word. If mis-recognition is likely, "
        "set clarify_suspected=true, put the corrected/intended meaning in "
